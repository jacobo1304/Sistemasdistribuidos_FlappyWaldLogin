	...
	public class AIBrain : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			if (Time.time - _lastDecisionsUpdate > DecisionFrequency)
			{
				...
				{
					// state changed, reset timer and skip actions this frame
					TimeInThisState = 0f;
				}
			}
			// perform actions only if state remained the same
			if (CurrentState == previousState && (Time.time - _lastActionsUpdate > ActionsFrequency))
			{
				CurrentState.PerformActions();
				_lastActionsUpdate = Time.time;
			}
			// update time in state and store position
